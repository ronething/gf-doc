...
随后我们可以通过请求`http://127.0.0.1:8199/api.v2/user/list`和随后我们可以通过请求`http://127.0.0.1:8199/api.v2/user/list?token=123456`来查看效果。
...
执行后，我们可以通过请求`http://127.0.0.1:8199/api.v2/user/list`和随后我们可以通过请求`http://127.0.0.1:8199/api.v2/user/list?token=123456`来查看效果，并查看终端的日志输出情况。

此外，有一个需要注意的细节是，我们这里的服务方法中使用了`panic`抛出一个异常错误。但是可以看到，该异常并没有中断中间件流程的执行。并且在终端也可以看到详细的错误堆栈信息。
