...
TOML致力于配置文件的小型化和易读性。
1. WIKI: https://github.com/toml-lang/toml/wiki
1. HOME: https://github.com/toml-lang/toml
...
1. 大小写敏感，必须是`UTF-8`编码
1. 注释：`#`
1. 空白符：`tab(0x09)`或`space(0x20)`
1. 换行符：`LF(0x0A)`或`CRLF(0x0D 0x0A)`
1. 键值对：同一行，无值的键不可用，每行只能保存一个键值对

TOML主要结构是键值对，与`JSON`类似。值必须是如下类型：
